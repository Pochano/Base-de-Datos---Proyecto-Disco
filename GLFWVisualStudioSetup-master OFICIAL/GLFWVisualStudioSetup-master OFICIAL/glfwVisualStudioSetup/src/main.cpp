...
// P�gina 1: Configuraci�n del Disco
int diskPlates = 0;
...
// P�gina 4 y 5: Tablas y Consultas
std::vector<std::vector<std::string>> queryResults;
std::string queryResultMessage = "";

// Entradas de la p�gina de consulta (estas son las variables que mencionaste)
...
// Funci�n auxiliar para manejar el contenido SQL simulado.
...
// Funci�n auxiliar para manejar el contenido CSV simulado.
...
void RenderPageConfiguration() {
    ...
    if (ImGui::Button("Guardar")) {
        if (diskPlates <= 0 || tracksPerPlate <= 0 || sectorsPerTrack <= 0 || sectorSize <= 0) {
            std::cerr << "Error: Todos los campos de configuraci�n del disco deben ser mayores que cero.\n";
        }
        else if ((sectorSize & (sectorSize - 1)) != 0) {
            std::cerr << "Error: El tama�o del sector debe ser una potencia de 2.\n";
        }
        else if (strlen(diskLocationBuffer) == 0) {
            std::cerr << "Error: La ubicaci�n del disco no puede estar vac�a.\n";
        }
        else {
            if (global_disco_manager) {
...
void RenderPageInsertTableSQL() {
    ...
    if (ImGui::Button("LEER TXT")) {
        if (!global_disco_manager) {
            sqlParseMessage = "Error: El disco no ha sido inicializado. Vuelve a la p�gina de Configuraci�n.";
        }
        else {
...
void RenderPageInsertTableCSV() {
    ...
    if (ImGui::Button("LEER .CSV")) {
        if (!global_disco_manager) {
            csvLoadMessage = "Error: El disco no ha sido inicializado. Vuelve a la p�gina de Configuraci�n.";
        }
...
void RenderPageQueryTables() {
    ...
    if (ImGui::Button("REALIZAR CONSULTA")) {
        ...
        std::cout << "[DEBUG] RenderPageQueryTables: Bot�n REALIZAR CONSULTA presionado." << std::endl;
        std::cout << "[DEBUG] RenderPageQueryTables: Select: '" << selectInput << "', From: '" << fromInput << "', Where: '" << whereInput << "'" << std::endl;


        if (fromInput.empty()) {
            queryResultMessage = "Error: La clausula FROM no puede estar vac�a.";
        }
            ...
            std::cout << "[DEBUG] RenderPageQueryTables: ExecuteSQLQuery retorn� " << queryResults.size() << " filas." << std::endl;

            if (queryResults.empty()) {
                queryResultMessage = "No se encontraron resultados o la consulta fue inv�lida.";
            }
            else {
    ...
    if (!queryResults.empty()) {
        if (ImGui::BeginTable("QueryResultsTable", static_cast<int>(queryResults[0].size()), ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_ScrollY, ImVec2(0, 200))) {
            for (size_t i = 0; i < queryResults[0].size(); ++i) {
                ImGui::TableSetupColumn(queryResults[0][i].c_str());
            }
...
void RenderPageShowTables() {
    ...
    if (!selectedTableToShow.empty() && global_disco_manager->tablas_cargadas.count(selectedTableToShow)) {
        ...
        headerRow.push_back("Ubicaci�n"); // A�adir la columna de ubicaci�n
        if (!headerRow.empty()) {
        ...
        if (!dir_filas_in.is_open()) {
            ...
            for (int row_idx = 0; row_idx < table.getCantidadFilas(); ++row_idx) {
                ...
                if (!std::getline(dir_filas_in, current_location_line)) {
                    current_location_line = "[Ubicaci�n Desconocida]";
                }
                ...
                if (!rowDataAny.empty()) {
                    ...
                    currentRow.push_back(current_location_line); // A�adir la ubicaci�n de la fila
        ...
        if (!displayData.empty() && displayData[0].size() > 0) {
            ...
            if (ImGui::BeginTable("DataTable", static_cast<int>(displayData[0].size()), ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_ScrollY, ImVec2(0, 300))) {
                for (size_t i = 0; i < displayData[0].size(); ++i) {
                    ImGui::TableSetupColumn(displayData[0][i].c_str());
                }
                ImGui::TableHeadersRow();
            ...
            std::cout << "[DEBUG] RenderPageShowTables: displayData est� vac�a o no tiene columnas." << std::endl;
        }
    }
    else {
...
int main(int, char**) {
    // Configuraci�n de errores de GLFW
    ...
    // Configuraci�n de la versi�n de OpenGL y GLSL
    ...
    // Configuraci�n del contexto de ImGui
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
    ...
    // Configuraci�n del estilo de ImGui
    ImGui::StyleColorsDark();

    // Cuando los viewports est�n habilitados, ajustamos WindowRounding/WindowBg para que las ventanas de plataforma se vean id�nticas a las normales.
    ...
    // Configuraci�n de backends de Plataforma/Renderizador
