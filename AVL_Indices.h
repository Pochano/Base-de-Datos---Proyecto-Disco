...
struct Nodo {
	...
	Nodo(std::string clave, int fila_ID) {
...
private:
	...
	Nodo* Insertar_Recursivo(Nodo* Nodo_R, std::string Clave, int Fila_ID) {
		...
		Nodo_R->Altura = 1 + std::max(Altura(Nodo_R->Izquierda), Altura(Nodo_R->Derecha));
	...
	Nodo* Buscar_Nodo_Recusivo(Nodo* Nodo_R, std::string Clave) {
	...
	void Busqueda_Rango_Recusiva(Nodo* Nodo_R, std::string Menor_Valor, std::string Mayor_Valor, bool Usa_Maximo, bool Usa_Minimo, std::vector<int>& Resultados) {
	...
	void Recorrido_InOrder_Recursivo(Nodo* Nodo_R) {
		...
		std::cout << Nodo_R->Clave << " : [";

		for (int i = 0; i < Nodo_R->Filas_ID.size(); i++) {

			std::cout << Nodo_R->Filas_ID[i];
...
public:
	...
	void Insertar(std::string Clave, int Fila_ID) {

		Raiz = Insertar_Recursivo(Raiz, Clave, Fila_ID);

	}

	std::vector<int> Buscar_Igualdad(std::string Clave) {
	...
	std::vector<int> Buscar_Rango(std::string Valor_Minimo, std::string Valor_Maximo) {
